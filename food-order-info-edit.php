...
if(isset($_SESSION['username']))
{
  ...
  $select_customer=mysql_query("SELECT * FROM foodorderinfo WHERE username='$account' AND paymentstatus='unpaid' AND status='pending' AND foi_id=(SELECT MAX(foi_id) FROM foodorderinfo)");
...
$data = getPost();
$insert_info = "UPDATE `foodorderinfo` SET `firstname` = '$data[0]', `lastname` = '$data[1]', `contactno` = '$data[2]', `address` = '$data[3]', `deliverydate` = '$data[4]', `deliverytime` = '$data[5]' WHERE `foodorderinfo`.`foi_id` = $qfoi_id";
try{
...
<div class="customer-box">
	...
	<form action="" method="POST" onsubmit="return Validate()" name="vForm">
			...
			<label id="firstname">Firstname</label><input type="text"  class="tField" name="firstname" value="<?php echo $qfname;?>" placeholder="Firstname *" pattern="[a-zA-Z]+" onkeyup="javascript:capitalize(this.id, this.value);"/><span><img class="star" src= "resources/star.png"/></span><div id="firstname_error" class="val_error"></div> 

			<br>
			<label id="lastname">Lastname</label><input type="text" class="tField" value="<?php echo $qlname;?>" name="lastname" placeholder="Lastname *" /><span><img class="star" src= "resources/star.png"/></span><div id="lastname_error" class="val_error"></div> 
			<br>
			<label id="contactno">Contact No </label><input type= "text" name="contact" value="<?php echo $qcontact;?>" onblur="checkLength(this)" placeholder="Contact Number *" onkeypress="return isNumberKey(event)" class="tField" maxlength="11"><span><img src= "resources/star.png"/></span><div id="contact_error" class="val_error"></div>
			<br>

			<label id="address">Address</label><input type="text" class="tAddress" name="address" value="<?php echo $qvenue; ?>" placeholder="Address *" /><span><img class="star" src= "resources/star.png"/></span><div id="address_error" class="val_errorAddress"></div>
			<br>
			<label id= "date1">Date</label><input type="text" id="txtDate2" name="date1" value="<?php echo $deliveryDate;?>" placeholder="Date of Delivery *" class="tField">
                            <span><img src="resources/star.png"/></span><div id="date1_error" class="val_error"></div>
                            <br>
            <label id= "time-1">Time</label>
        <select placeholder="--Select Time--" class= "time1" value="<?php echo $deliveryTime;?>" id= "time1" name="time1" onsubmit="return Validate()">
...
<script type="text/javascript">
	// GETTING ALL INPUT TEXT FIELDS
        ...
        var timeSelected = $("#time1 option:selected").val()
