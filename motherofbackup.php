...
$select_menulist=mysql_query("SELECT * from menureserved WHERE username='$account' && status='pending' && paymentstatus='unpaid' && mr_id=(SELECT MAX(mr_id) FROM menureserved WHERE username='$account')");
  ...
  $select=mysql_query("SELECT pax from reservationlist WHERE username='$account' && paymentstatus='unpaid' && rsv_id=(SELECT MAX(rsv_id) FROM reservationlist)");
...
date_default_timezone_set("Asia/Bangkok");
    ...
    $date = date("Y-m-d");
    $status = "pending";
    $query = "INSERT INTO bpipayment VALUES(NULL, '$amount','$imagetmp','$imagename','$date', '$status', '$account')";
...
<div class="pax-box">
  <p class="heading"><strong>Package Information</strong></p>
  <?php
  echo "<table class='tbl-head' cellpadding='10' cellspacing='1'>";
  echo "<tbody>";
