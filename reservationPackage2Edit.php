...
$stat = "pending";
$paymentStat = "unpaid";
$mr_id_Q = $_SESSION['mr_id'];
$insert_Menus = "UPDATE `menureserved` SET `beef` = '$data[1]', `fish` = '$data[2]', `chicken` = '$data[3]', `noodles` = '$data[4]', `vegetables` = '$data[5]', `rice` = '$data[6]', `dessert` = '$data[7]', `drinks` = '$data[8]' WHERE `menureserved`.`mr_id` = $mr_id_Q;";
 try
    {
...
$status = "pending";
$pkgrsv_id_q = $_SESSION['pkgrsv_id'];

    $insert_Query = "UPDATE `packagereserve` SET `pkg_name` = '$data[0]', `op1` = '$data[1]', `op2` = '$data[2]', `op3` = '$data[3]', `op4` = '$data[4]', `total` = '$data[5]' WHERE `packagereserve`.`pkgrsv_id` = $pkgrsv_id_q;";
   ...
   try
    {
        ...
        if($insert_Result)
        {
            if(mysqli_affected_rows($connect) > 0)
            {
              ...
              echo "<script>alert('Package Updated!');document.location='reservationPayment.php'</script>";
...
<div class="content-reservation">
 <h1 class="big-title">- Reservation <?php echo $_SESSION['pkgrsv_id'];?> -</h1>
    <hr width="900"></hr>
  <ul class="tab">
